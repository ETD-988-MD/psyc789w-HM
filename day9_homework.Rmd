...
see.the.dist <- function(data){
  ...
  for(i in 1:length(data)){
    ...
    if(is.numeric(data[,i])){
      sub <- as.numeric(data[,1])
      } else {is.character(data[,i]) | is.factor(data[,i]))} if(!is.factor){
        y <- as.factor(data[,i])
        plot <- ggplot(data,aes(x=y)) + 
      geom_histogram() + theme_bw() +
      xlab(col.names[i]) + ggtitle(paste("Variable:",col.names[i])) 
    assign(paste("plot",i,sep="_"),plot)
        } else {
          f <- data[,i]
          plot <- ggplot(data,aes(x=f)) + 
      geom_histogram() + theme_bw() +
      xlab(col.names[i]) + ggtitle(paste("Variable:",col.names[i])) 
    assign(paste("plot",i,sep="_"),plot)}
  plot <- ggplot(data,aes(x=sub)) + 
  geom_histogram() + theme_bw() +
  xlab(col.names[i]) + ggtitle(paste("Variable:",col.names[i])) 
assign(paste("plot",i,sep="_"),plot)
}
