...
```{r}
#My initial approach works
x<- mob.sub %>% group_by(country,ID) %>% mutate(min=min(year)) %>% mutate(count = year - min)
```

```{r,eval=F}
#But things gets more complicated when making the counter dependent on the war variable

mob.sub %>% 
  group_by(country,ID) %>%
  mutate() %>%
  mutate(years.since.last.war = year -max(year)) 
