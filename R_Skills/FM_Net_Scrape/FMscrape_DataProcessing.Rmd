...
temp.data$type.of.meeting <- ifelse(str_detect(corpus[[3]]$meta[[4]] %>% tolower,"telephone"),"Telephone",ifelse(str_detect(corpus[[3]]$meta[[4]] %>% tolower,"meeting"),"Meeting",NA))
temp.data$date <- corpus[[3]]$meta[[8]] %>% as.Date %>% format(.,"%m-%d-%Y")
temp.data$origin <- corpus[[3]]$meta[[7]]
temp.data$origin.source <- corpus[[3]]$meta[[1]]
temp.data$corpus.location <- corpus[[3]]$meta[[5]]
View(temp.data)
```
...
for(i in 1:length(corpus)){
  if(is.na(corpus[[i]]$meta[[3]])){
    content <- tm_map(corpus, content_transformer(tolower)) %>%  .[i]
    s <- content[[i]]$content %>% as.character
  } else{
    s <- corpus[[i]]$meta[[3]] %>% as.character %>% tolower()
  }
}
